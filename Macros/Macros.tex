\newcommand{\errSym}[1]{$\pm$ #1}
\newcommand{\errAsym}[2]{$^{+#1}_{-#2}$}
...
\newcommand{\ZZllvv}{\zzllvv}

\newcommand{\eeee}{\ensuremath{e^{+}e^{-}e^{+}e^{-}}}
\newcommand{\eemumu}{\ensuremath{e^{+}e^{-}\mu^{+}\mu^{-}}}
\newcommand{\mumumumu}{\ensuremath{\mu^{+}\mu^{-}\mu^{+}\mu^{-}}}

\newcommand{\llvv}{\ensuremath{\ell^{-}\ell^{+}\nu\bar{\nu}}}
\newcommand{\llll}{\ensuremath{\ell^{-}\ell^{+}\ell^{-}\ell^{+}}}
\newcommand{\ZZs}{\ensuremath{ZZ^{*}}}
\def\ggZZ{{\sc gg}2{\sc zz}\xspace}

\def\ossf{opposite-sign same-flavour}
\def\met{missing transverse energy}

\def\ZZs{$\ZZ^{*}$}
\def\missET {$E_{\mathrm{T}}^{\mathrm{miss}}$}
\def\invfb {fb$^{-1}$}
\def\zzllvv {$ZZ\ra\ll\nu\bar{\nu}$}
\def\zzllll {$ZZ\ra\ll\ell'^{+}\ell'^{-}$}
\def\zzsllll {$ZZ^{*}\ra\ll\ell'^{+}\ell'^{-}$}
\def\llvv {$\ll\nu\bar{\nu}$}
\def\llll {$\ll\ell'^{+}\ell'^{-}$}
\def\ztt {$Z\ra\tau^{+}\tau^{-}$}
\def\sqrts7{\ensuremath{\sqrt{s} = 7 } TeV}

%\def\Zll {$Z\ra\ell^{+}\ell^{-}$}
%\def\Wlv {$W\ra\ell^{\pm}\nu$}

\def\amcatnlo{aMC@NLO\xspace}
\def\sherpa {{\sc Sherpa}}
\def\jimmy {{\sc Jimmy}}
\def\ggtwozz {{\sc gg2zz}}
\def\madgraph {{\sc Madgraph}}
\def\powhegbox {{\sc PowhegBox}}
\def\mcfm {{\sc mcfm}}

\def\lumi {4.64 \ifb\ }
\def\fidllll {25.4$^{+3.3}_{-3.0}$ (stat.) $^{+1.2}_{-1.0}$ (syst.) $\pm$ 0.4 (lumi.) \fb}
\def\fidsllll {29.8$^{+3.8}_{-3.5}$ (stat.) $^{+1.7}_{-1.5}$ (syst.) $\pm$ 0.5 (lumi.) \fb}
\def\fidllvv {12.5$^{+3.0}_{-2.8}$ (stat.) $^{+1.0}_{-1.1}$ (syst.) $\pm$ 0.2 (lumi.) \fb}
\def\totzz {6.7$^{+0.7}_{-0.7}$ (stat.) $^{+0.4}_{-0.4}$ (syst.) $\pm$ 0.1 (lumi.) \pb}
\def\theoryzz {6.18$^{+0.25}_{-0.18}$ \pb}
\def\theoryzzmass {5.89$^{+0.22}_{-0.18}$ \pb}
...
\def\Z{\ensuremath{\Zboson}}
\def\W{\ensuremath{\Wpm}}
\def\mcatnlo{MC@NLO\xspace}
\usepackage{xspace}
\def\WW{\ensuremath{\Wplus\Wminus}\xspace}
\def\WZ{\ensuremath{\Wpm\Zzero}\xspace}
\def\WZl{\ensuremath{\Wpm\Zzero}$\rightarrow \ell^\pm \nu \ell^+\ell^- $\xspace}
\def\Wg{\ensuremath{\Wpm\gamma}\xspace}
\def\Zg{\ensuremath{\Zzero\gamma}\xspace}
\def\ZZ{\ensuremath{\Zzero\Zzero}\xspace}
\def\Zorgv{\ensuremath{\Zzero/\gamma^{\textstyle *}}\xspace}
\def\madgraph{{\sc MadGraph}\xspace}
\def\pythia{{\sc Pythia}\xspace}
\def\pythiaB{{\sc PythiaB}\xspace}
\def\mcatnlo{MC@NLO\xspace}
\def\ggWW{ggWW\xspace}
\def\alpgen{{\sc Alpgen}\xspace}
\def\alpgenJimmy{{\sc Alpgen/Jimmy}\xspace}
\def\herwig{{\sc Herwig}\xspace}
\def\Jimmy{Jimmy\xspace}
\def\BHO{BHO\xspace}
\def\BosoMC{BosoMC\xspace}
\def\ggtwoww{{\tt gg2ww}\xspace}
\def\ptsp1{\ensuremath{p_T} }
