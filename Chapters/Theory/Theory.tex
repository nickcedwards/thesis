...
constants of motion in a quantum-mechanical system, e.g. a particles charge or
...
charge and a coupling strength of $\alpha=1/137$. The photon has zero
mass, and so electromagnetism is a long range force, and is chargeless so
does not self interact. The theory describing the electromagnetic force in the \sm\ is known
...
\begin{equation}
\Lagr = \bar{\psi} ( i \gamma^\mu \partial_{\mu} ) \psi
\end{equation}

where $\bar{\psi = \psi\bar\gamma^{0}}$ is the conjugate field. In order to
make this Lagrangian invariant under the \uone\ and \sutwo\ gauge groups it is
...
Substituting Equations~\ref{eqn:pauli-matrices,eqn:hypercharges,eqn:ew-covariant-derivative}-\ref{eqn:hypercharges}
...
should not depend on the choice of the scales \uR\ and \uF; however since in LO and NLO
