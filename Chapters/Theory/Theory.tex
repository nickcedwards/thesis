...
mechanism} by introdcuing a doublet of complex scalar fields (the \intro{Higgs
Field}) which has a non-zero vaccuum expectation value. The gauge bosons aquire
...
fundamental particle, the \intro{Higgs Boson}, a massive scalar boson. There has
...
where $I_{3}(f)$ is the third component of weak isospin and $Y(f)$ is the
...
Substituting Equations~\ref{eqn:ew-covariant-derivative}, \ref{eqn:hypercharges}
and~\ref{eqn:pauli-matrices} into~\eqn{ew-lagrangrian} gives the following
interaction terms between the leptons and the gauge bosons:

% 5.34 from book
\begin{equation}
- \frac{g}{2} \left( \begin{array}{cc}\overline{\nu_{L}} &  \overline{\eL}
\end{array} \right) \gammamu \left(
\left( \begin{array}{cc} \Wthreemu & \sqrt{2} \Wpmu \\ \sqrt{2} \Wmmu & -
\Wthreemu \end{array} \right)
- \tan{\thetaW} \right)
\left( \begin{array}{c} \nu_{L} \\  \eL \end{array} \right)
- i g \tan{\thetaW} \overline{\eR}\gammamu \Bmu \eR
