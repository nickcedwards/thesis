...
There are two types of fermions: \intro{quarks} and \intro{leptons}. The quarks are the
...
Each generation of leptons consists of an electrically neutral \intro{neutrino}
...
as \intro{Quantum Electrodynamics (QED)}.
...
as \intro{Quantum Chromodynamics (QCD)}.
...
%\subsection{The Dirac Equation and Spinors}

%The Dirac equation describes relativistic quantum wave functions for fermions.
%The Lagrangian for the Dirac equation is

%\begin{equation}
%\Lagr = \bar{\psi} ( i \gamma^\mu
%\end{equation}
...
invariant under \intro{local gauge transformations}, that is that the Lagrangian
...
vector \intro{gauge fields} with associated \intro{gauge bosons}. Interactions
...
%\ref{eqn:ew-covariant-derivative}-\ref{eqn:pauli-matrices}
...
Quantum Chromodynamics is the theory of the strong interactions, and is a
non-abelian gauge theory with gauge group \suthree. The charge associated with
\suthree\ is the \intro{colour charge}, so named because there are three colour
...
local gauge transformations generated by the group. This is again achieved by replacing the derivate in the Dirac equation
...
of $W$ bosons in the weak interaction, the last term in~\eqn{colour-field-strength} gives rise to triple and quartic interactions
...
Since the gluons are massless, QCD contains UV
...
parameters. Renormalised parameters such as \alphaS\ are dependent on the  \intro{renormalisation scale} \uR, the
scale at which this subtraction is done. Another result of this procedure is that
\alphaS\ becomes dependant on the momentum squared of the external particles
coming into a vertex, $Q^{2}$:
\begin{equation}
\alphaS(Q^{2}) = \alphaS(\uR^{2}) - \alphaS(\uR^{2}) \beta_{0} \ln(Q^2{}/\uR^{2})
\end{equation}
Since the parameter $\beta_{0}$ is found to be positive, the effective coupling
{\it decreases} as the momentum scale $Q$ increases, a phenomina referred to as
the \intro{running of the coupling}. For momentum scales below
$\sim2$ \gev, \alphaS\ becomes large, and it is not possible to perform
perturbative calculations in QCD and in this \intro{non-perturbative} regime
phenomenological models are needed.

The running of the coupling gives rise to two additional phenomena:
\intro{confinement} and \intro{asymptotic freedom}. Since \alphaS\ increases as
$Q$ decreases, the strong force between two quarks increases as the distance
between them increases, leading to stronger and stronger binding as they are
pulled apart. At sufficiently far separation, the colour `string' connecting the
...
protons themselves that interact but their quark and gluon constituents. The interaction between two \intro{incoming} partons (quarks or gluons) from the two initial
...
hadrons is the \intro{factorisation theorem}~\cite{ellis2003qcd}, which allows the hard
