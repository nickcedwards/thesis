...
N^{obs}_{\ell\ell\ell'\ell'} - N^{bkg}_{\ell\ell\ell'\ell'}}{\mathcal{L} \times
\CZZ^{\ell\ell\ell'\ell'}}
\end{equation}
where $N^{obs}_{\ell\ell\ell'\ell'}$ and $N^{bkg}_{\ell\ell\ell'\ell'}$ are the number of observed and
...
in~\sec{objSel:CZZ}, for the $\ell\ell\ell'\ell'$ final-state.  
