...
\begin{figure}[htbp]
    ...
    in candidate \ZZ\ events in the 7~\tev\ data. In each plot, the \Z\ boson candidate not shown in the
    plot is required to pass the \sstoos\ requirement. The points represent the observed data and the
    histograms show the prediction from simulation, where the background is
    normalised to the total background estimate as described in~\chap{BackgroundEstimate}. 
...
\begin{figure}[htbp]
    ...
    {Invariant masses of the (a) leading and (b) subleading \leppair\
    in candidate \ZZ\ events in the 8~\tev\ data.  In each plot, the \Z\ boson candidate not shown in the
    plot is required to pass the \sstoos\ requirement.  The points represent the observed data and the
    histograms show the prediction from simulation, where the background is
    normalised to the total background estimate as described in~\chap{BackgroundEstimate}. 
    The shaded band shows the combined statistical and
