...
these figures this detector distortion is included in the \mc\ simulation, alowing a comparison of the
theoretical prediction and the observed data, it is desirable to correct the
observed distributions for detector effects in order to allow model independant
...
distributed according to a \probDF\ $f(x)$. Due to experimental effects, one does not measure $x$ but a different variable $y$ distributed
...
\end{equation}
In practice, one tends not measure continuos distribution but makes measurements
in discretised bins, i.e. histograms. $x$ and $y$ can thus be rewritten as
...
matrix \unfA\ as a description of the probability of observed data given the
true distribution. \unfA\ is determined from \mc\ simulation. The
algorithm takes an initial prior for the simulated distribution of \unfx\ from
...
distribution, but this bias decreses steeply with the number of iterations. The
statistical uncertainty tends to increase with the number of iterations, as
fluctuations can be amplified by positive feedback inherant in the algorithm;
...
Statistical uncertainties are assesed by
...
pseud-experiments is taken as the statistical uncertainty. Systematic
uncertainties due to experimental uncertainties are handled in a similar manner,
re-running the unfolding procedure multiple times, varying the repsonse matrix
...
in~\fig{unfolded-distributions}; the \sm\ prediction is consistent with the
