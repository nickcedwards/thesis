...
expected 95\% confidence interval limits obtained using the 7~\tev\ and the 8~\tev\ data,
respectively. The limit on each coupling assumes the other couplings are fixed
at their \sm\ value of zero. In both datasets, no evidence for \TGC s is observed, and the
observed limits are consistent with the expected limits. The expected limits
using the 8~\tev\ data are 2.4-3.0 times more stringent than the expected limits
using the 7~\tev\ dataset using both the \ZZlll\ and \ZZllvv\ channels, or
3.0-3.6 times more stringent than the expected limits using only the \ZZlll\
channel of the 7~\tev\ data.

\figs{TGC-limits-2D-seven}{TGC-limits-2D-eight} show `two-dimensional' 95\%
...
A comparison of the limits presented here those 
derived from LEP, Tevatron and CMS data is shown in figure~\ref{fig:tgccomb}.
The one-dimensional limits are more stringent than those derived from measurements at
LEP~\cite{bib:LEPEW2006} and the Tevatron~\cite{bib:D0_ZZ1} and previously by ATLAS~\cite{ATLAS_ZZ4l:1fb2011};
it should be noted that
the limits from LEP do not use a form factor, and those from the Tevatron use
$\Lambda = 1.2\TeV$.
...
\end{figure}
 ...
 \begin{center}
  ...
  \caption{\small 
Anomalous nTGC 95\% confidence intervals from ATLAS, CMS~\cit{}, LEP~\cite{bib:LEPEW2006} and Tevatron~\cite{bib:D0_ZZ1}
