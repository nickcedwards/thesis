...
95\% confidence level (CL) intervals for the \TGC s are determined using a
maximum profile likelihood method on the number of observed events in each bin
of the differential distributions, with systematic uncertainties introduced as
nuisance parameters. The three \lllplp\ final states are each merged prior to
the maximum profile likelihood maximisation. The likelihood function is similar
to that used in the cross section calculation, but parametrised in terms of the
number of events in each bin $m$ instead of the \cx.  Nuisance parameters are
again represented as with Gaussian terms ($G$); since there are nuisance
parameters for both signal and background, 
