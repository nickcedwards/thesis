...
\TGC\ parameters is calculated as the fraction of pseudo-experiments which have a test
statistic smaller than the observed value of the test statistic $\teststat_{obs}$.
This procedure is repeated scanning possible values of $\vec{f}$, and the 95\%
confidence interval is defined by all values of $\vec{f}$ for which
$p(\vec{f})\geq 5\%$. Limits are set for one parameter at a time, holding the other parameters at
their \sm\ values of zero (termed \intro{one-dimensional limits}, and  also
varying two parameters simultaneously (\intro{two-dimensional limits}).

The expected sensitivity is obtained using
