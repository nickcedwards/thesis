...
denominator, and $\vec{\hat{f}}$ and $\vec{\hat{\beta}}$ are the values of
...
parameters is obtained by generating 10,000 pseudo experiemnets. In generating
the pseudo experiements, the nuisance parameters $\vec{\beta}$ are gaussian
fluctuated around the mean value of $\vec{\betahathat}$ and the number of events
are drawn randomly from a Poisson distribution with a mean corresponding to the
...
statistic smaller (i.e. indicating less compatibility with the test values of
the couplings) than the observed value of the test statistic $\teststat_{obs}$.
