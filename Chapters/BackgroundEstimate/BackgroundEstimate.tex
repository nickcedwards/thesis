...
Denomiting true leptons as $T$ and background leptons as $B$, the total background due
...
requirements. Of course, given a selected lepton in the data it is impossible to
...
two new definitions are introduced: selected leptons, denoted $L$, that
pass all of the lepton selection requirements; and lepton-like-jets $J$, which
...
two or three \sellep\ and one one or two \lljet (s), by means of a \ffactor
\FF, defined as the ratio of the probability for a background lepton to be
...
\end{equation}
The number of events with two \lljet s and two \sellep\ can be related to the
