...
test is performed. The \ffactors\ are derived from the \mc, and are then applied
to $LLJJ$ and $LLLJ$ events from the \mc\ samples modelling \Zjets, \ttbar,
\singletop, \WZ\ and \WW\ backgrounds, to give a `background estimate' according
to the \mc. This is then compared to the number of $LLLL$ events passing the full
selection in the \mc\ samples listed above. If the method were perfect, and
given sufficient statistics, these two numbers would agree.
