...
calorimeter response. ?? Say something about being non compensating ??. First
the topological clusters are corrected to the \intro{EM scale}, corrections
applied so that the response to purely electromagnetically interacting particles
...
clusters to correct for the effects of \pu. These are derived from minimum-bias
data as a function of the number of primary vertices in the event, and subtract
the average additional energy deposited by \pu\ events in a fixed grid of
\deltaetadeltaphi{0.1}{0.1}. The clutser four momentum is modified so that each
topological cluster points to the primary vertex with the highest sum of track
transverse momentum. Finally  the \intro{Jet Energy Scale (JES)} correction is
applied to correct for the non compensating nature of the calorimeters, and also
to correct for the effects of dead material, leakage outside of the
calorimeters and energy lost due to particles being defelected out of the jet by
the magnetics fields. This correction is applied at jet level, paramaterised by
the jet \pt\ and $\eta$. It is primarily derived from \mcsim\ by comparing the
...
$Z\plus$jet balance~\cite{ATLAS-CONF-2012-053} and $Z \plus \gamma$ balance~\cite{ATLAS-CONF-2012-063} are used to validate the \mc\
based corrections.
