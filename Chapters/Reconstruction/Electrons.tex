...
highest \et\ cell in the middle calorimeter layer in the RoI is used as a seed
rather than using a sliding window algorithm. Basic shower shape cuts on
the width in $\eta$ and the ratio of energy deposits in the different layers are
used to reject backgrounds. The EF uses the offline reconstruction and
identification algorithms as described in~\sec{reco-el-reco}
and~\sec{reco-el-id}, although slightly looser cuts are applied to remain fully
efficient offline. It uses some information from just outside the RoI.

The bandwidth dedicated to electron and photon is approximately 30\% of the
...
an acceptable level. At the start of 2011 the primary single electron trigger
has a threshold of 20 \gev. When the instantaneous luminosity exceeded
2\timetenpower{33}\instlumiunit\ the threshold was increased to 22 \gev\ and as
the luminosity increased to 3\timetenpower{33}\instlumiunit\ the identification
...
leakage cut was also introduced to further reduce the L1 rate. 
