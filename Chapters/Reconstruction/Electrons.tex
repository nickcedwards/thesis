...
The baseline EM calorimeter energy scale is derived from test beam
measurements. This introduces large uncertainties originating from the transfer
...
pseudo-rapidity. As previously mentioned, \mc\ derived calibrations are applied
to the clusters to correct for dead material and leakage outside of the
calorimeter. The energy response is further corrected in-situ using \Zee\
...
measures the residual mis-calibration. The $\alpha$ parameters are obtained by
performing an unbinned fit minimising the log-likelihood function:

\begin{equation}
-{\rm ln } L_{\rm tot} = \sum_{i,j} \sum^{N^{\rm events}_{ij}}_{k} -{\rm ln }
L_{ij}(\frac{m_{k}}{1+\frac{\alpha_{i} + \alpha_{j}}{2}})
\end{equation}

minimising the  the observed dilepton invariant mass distribution in a sample of
opposite-signed high \pt\ electrons with invariant mass near the \Z\ boson mass,
and requiring the observed \Z\ mass to be consistent with the world average measured
value~\cite{PDG}. The procedure is carried out in bins of pseudo-rapdity.
~\fig{el-energy-calib-2011} shows the 

\begin{figure}[h]
\centering
            \includegraphics[width=0.8\textwidth]{zee_mass_calibration_2011}
\caption{. Figures from~\cite{2}.}
