...
\begin{itemize}
    ...
    \item {\bf Segment Tag:} combination of an ID track with an MS track
...
It is also possible to reconstruct muons independently of the MS. The 
\CaloTrkMuID\ algorithm uses the calorimeter to tag inner-detector tracks as originating from muons.
 A muon
with sufficient momentum will traverse all the layers of the calorimeter,
leaving a small energy deposit in each layer. It is possible to distinguish this
pattern of energy depositions from that of other particles. A low
...
deposit much energy in the core cells. Electrons will deposit most of their energy
in the EM calorimeter. The \CaloTrkMuID\ algorithm requires a signal consistent
with a minimum-ionising-particle in each layer of the
calorimeter to reject hadron and electron backgrounds. Both a cut based quality selection and a likelihood
