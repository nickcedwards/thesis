...
as well as the efficiency predicted by the Monte-Carlo simulation. The
...
the Monte-Carlo to reproduce the efficiency observed in data.
...
\centering
    ...
    pseudorapidity and the transverse momentum of the muon for muons reconstructed
    as either Combined or Segment Tagged using the \staco\ algorithm. ]{Muon
    ...
    efficiency predicted by Monte-Carlo simulation. In figure (a) the efficiencies
    for calorimeter tagged muons are also shown for \modetalt{0.1} (solid blue
    triangles for data and open green triangles for Monte-Carlo). Figures
