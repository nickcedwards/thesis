...
detector. The ID muon momentum resolution can thus be parameterised as:
...
resolution parameters: a series of di-muon mass distributions are produced for
...
\begin{equation}
\Delta(MS) = f(0,1) \Delta p_{1}^{MS} +  f(0,1) \Delta p_{2}^{MS} p_{T}
\end{equation}

and the $ \Delta p_{i}^{MS} $ are the difference in the resolution parameters $
p_{i}^{MS} $ between data and \mcsim.  The ID resolution in simulation is
