...
damage, it is necessary to keep the SCT modules at a temperature of around
$-7^{\circ}$C; this is discussed in more detail below. The SCT uses an evaporative cooling system
to keep the modules at this temperature; this is described in Section ?.
...
damage and the bias voltage must be increased to compensate. As described in the
previous section, the silicon detectors must be run
...
evaporative cooling is used to keep the modules at this temperature~\cite{1748-0221-3-07-P07003}. An
evaporative cooling system was chosen over a mono-phase system as it has a
larger cooling capacity per unit volume do to utilisation of the latent heat of
vaporisation rather than a liquids heat capacity. This allows the resulting
system to be smaller and with a lower mass, thus contributing less to the
material budget of the detector. Additionally, an evaporative system has
smaller temperature gradients along long cooling tubes, allowing for a more
