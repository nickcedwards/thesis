...
designed and that the modules are still coupled to the cooling structures.  This
...
an additional thermistor mounted on the back side of the module.  The temperature difference between the front and the back of barrel
...
~\fig{evap-cooling} shows a schematic of the cooling system.
The system uses a fluorocarbon refrigerant C$_3$F$_8$ as a cooling fluid, chosen since it is stable against
irradiation, non-flammable, non-toxic, electrically neutral, and has the highest
heat transfer coefficients compared to similar fluorocarbon refrigerants. The
cooling fluid is delivered at room temperature in saturated
liquid phase to capillaries
...
modules by the passing fluid causing it to evaporate. At the end of the cooling
pipe a heater evaporates any any residual liquid and heats the vapour to above
the cavern dew point to avoid condensation in the return pipes. Recuperative
heat exchangers transfer heat between the warm inlet liquid and the cold return
vapour, increasing the efficiency of the thermodynamic cycle by decreasing the
vapour quality of the fluid at the inlet to the cooling structure. 
The exhaust gas is piped to the ATLAS service cavern where is compressed and
condensed, and then returned to the cooling loop.
The temperature of the detector structures depends on the boiling pressure
of the fluid, which is controlled by backpressure regulators (BPR) located at
the end of the return tubes.
...
\section{Methodology}
The temperature of the SCT modules are monitored by thermistors mounted on the
hybrid.
...
database.  It is necessary to first find periods of stable running where the
cooling system is turned on and the system in a stable state. Stable periods are
...
veto periods where the detector is being turned on and off. Stable periods
...
the barrel layers and endcap disks. Modules which fall in the far outlying
regions of the distributions are identified as `problem' modules. A list of
problem modules is maintained, and these modules are monitored. The plots are
produced automatically on a daily basis by scripts running on one of the SCT
monitoring computers. The plots are made available as part of the `SCT
...
SCT barrel modules have two thermistors, one mounted on either side of the
module. The difference in temperature between the front and back of the module
...
apart from one another. The monitoring variable \deltat is defined as
...
Figure~\ref{fig:tdiff_dist} shows the distributions of \tdiff\ for the barrel and
for each of the endcaps for 01:20 22/9/2012 to 01:20 23/9/2012. As expected the
  mean of the distribution is consistent with zero. Endcap C shows a large tail
  to low \tdiff, indicating there are a large number of modules warmer than
...
width of the distributions of the monitoring variables are
produced. Such plots allow identification of long term trends common to all
modules, which would not be identified by the procedure of identifying
`problematic' outliers described in the previous sections.
...
for one stable period. The mean and width of the distribution is seen to be
