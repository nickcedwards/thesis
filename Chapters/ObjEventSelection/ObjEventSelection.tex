...
are summarised in~\tab{objsel-el} and are described in more detail below. 
...
\small
  \begin{tabular}{ l  l l }
    \hline\hline 
      ...
      ID Selection                & Tight & \\
      $\eta$                & \modetaclusterbetween{2.5}{3.16} &  \\
      $E_T$                 & $E_T > 20$ GeV & \\
...
frequently producing high noise or failing to give a readout. There were also
...
\deltaRlt{0.2}\footnote{where \deltaRdef} be less than 15\% of the electron's \pt, i.e.
...
\ZZ\ events are shown in~\tab{lepseleff-CentralEl}.
...
\ZZ\ events are shown in~\tab{lepseleff-ForwardEl}.
...
reconstructed electrons within the $\eta$ acceptance (\modetabetween{2.47}{3.16}). The second 
...
object within \deltaRlt{0.1} and have \ptgt{25}, and they must have invariant mass satisfying \sstooos. Any additional electrons in the event are required to pass
...
summarised in~\tab{objsel-mu} and are described in more detail below. 

\subsection{Central Muon Selection}

Central muons may be either \combined\ (with a full MS and ID track) or
\segmentTagged\ (with a full ID track but only track segment in the MS), and have $\pt>7$ GeV
and \modetalt{2.47}.
...
\ZZ\ events are shown in~\tab{lepseleff-CentralMu}.
...
in events which fall in the \ZZ\ fiducial volume at generator level. The first
and third columns show cumulative efficiencies with respect to the number of
reconstructed muons within the $\eta$ acceptance (\modetalt{2.5}), for
the 7~\tev\ and 8~\tev\ analyses respectively. The second and fourth
columns show the efficiency with respect to the previous requirement.}
...
ID track are termed `\standAlone'.~\fig{fwdmu-frac-combined} shows the fraction
...
muons which have an ID track and are \combined\ muons, as a function of
...
forward muons in
\ZZ\ events are shown in~\tab{lepseleff-ForwardMu}.
...
in events which fall in the \ZZ\ fiducial volume at generator level. The first
and third columns show cumulative efficiencies with respect to the number of
reconstructed muons within the $\eta$ acceptance (\modetabetween{2.5}{2.7}), for
the 7~\tev\ and 8~\tev\ analyses respectively. The second and fourth
columns show the efficiency with respect to the previous requirement.}
...
The estimated efficiencies of the calorimeter tagged muon selection requirements
for calorimeter tagged muons in
\ZZ\ events are shown in~\tab{lepseleff-CaloMu}.
...
in events which fall in the \ZZ\ fiducial volume at generator level. The first
and third columns show cumulative efficiencies with respect to the number of
reconstructed muons within the $\eta$ acceptance (\modetalt{0.1}), for
the 7~\tev\ and 8~\tev\ analyses respectively. The second and fourth
columns show the efficiency with respect to the previous requirement.}
...
the selection requirements described above, and one must match to the triggering
object within \deltaRlt{0.1} and have \ptgt{25}. Any additional muon candidates in the event are required to pass
...
fairly well, the number of additional muon candidates observed in data is much higher than
...
single-muon triggers, as described in Section~\ref{sec:reco-el-triggers}
...
\begin{tabular}{lccc p{5cm}}
\hline \hline
Period & \Et\ Threshold (GeV) & Isolation Requirement & Int. Luminosity (\ifb) \\
\hline
\multicolumn{3}{l}{ \bf 2011 (7~\tev\ data) } \\
B - I & 20 &  None & 1.65 \\
J - K & 22 &  None & 0.91 \\
L - M & 22 &  None & 2.76 \\ 
%L - M & \texttt{e22vh\_medium1} & 22 & 2.76 & Variable L1 thresholds; hadronic leakage requirement \\ 
\hline
\multicolumn{3}{l}{ \bf 2012 (8~\tev\ data) } \\
A - M & 24 &  \ptconetwentylt{0.10}  & \LumiTotalReadyTwentyTwelve \\
\hline\hline
\end{tabular}
\end{center}
\caption{Electron triggers used in the different data taking periods.}
...
\begin{tabular}{lccc p{5cm}}
\hline \hline
Period & \pt\ Threshold (GeV) & Isolation Requirement & Int. Luminosity (\ifb) \\
\hline
\multicolumn{3}{l}{ \bf 2011 (7~\tev\ data) } \\
B - I & 18 &  None & 1.65 \\
J - M & 18 & None & 3.67 \\
...
\hline
\multicolumn{3}{l}{ \bf 2012 (8~\tev\ data) } \\
A - M & 24 & \ptconetwentylt{0.12} & \LumiTotalReadyTwentyTwelve \\
\hline\hline
\end{tabular}
\end{center}
\caption{Muon triggers used in the different data taking periods.}
...
object that caused the trigger to fire with \deltaRlt{0.1}. Such leptons are referred to as 
...
decays is used. \Zee\ and \Zmm\ events are selected by requiring that events have at
...
shows the same distributions for the 8~\tev\ data. \figs{dilep-lepkin-seven}{dilep-lepkin-eight}
show the lepton \pt, $\eta$ and $\phi$ for the 7~\tev\ and 8~\tev\ data,
respectively. The \mc\ samples used to describe the various processes are as listed
in~\tab{mcbg-generators}, except that the \WZ\ process is modelled using
\herwig\ as \WZ\ the samples listed in~\tab{mcbg-generators} do not include hadronic
decays of the \W\ boson.
Good agreement is seen
between the data and the \mcsim. A slight excess in the data for the \dielectron\
...
\begin{itemize}
    ...
    \item {\bf Leading and Subleading:} The \Z\ candidate with the larger \pt\
    of the two is referred to as the \intro{leading} \Z; the \Z\ candidate with the
    lower \pt\ is referred to as the \intro{subleading} \Z.

    \item {\bf Primary and Secondary:} The \Z\ candidate closer to the \Z\
...
\begin{enumerate}
    ...
    is estimated from \mc\ to be 99.6\%. No events failing this cut were
...
in~\tab{objSel-cutflow-eight}. The numbers are obtained from the nominal
\powhegbox+\ggtwoZZ\ samples, described in~\sec{nominal-signal-sample}.
...
$N_{ZZ}$ (7~\tev, \LumiPassGRLTwentyEleven\ \ifb)	  & \eeee\ & \mmmm\ & \eemm\ & \llll\  \\
  ...
  %$M_{34}>$20~\gev\        &  12.51 $\pm$ 0.15 &  20.35 $\pm$ 0.21  &  32.50 $\pm$ 0.27 &  65.37 $\pm$ 0.37   \\ 
   %66$<M_{34}<$116~\gev\    &  10.28 $\pm$ 0.14 &  16.45 $\pm$ 0.19  &  26.67 $\pm$ 0.24 &  53.41 $\pm$ 0.34   \\

         Four Leptons &  $18.08 \pm 0.19$ &  $34.33 \pm 0.27$ &  $49.09 \pm 0.32$ &  $101.49 \pm 0.46$ \\
          Quadruplet &  $17.20 \pm 0.19$ &  $34.31 \pm 0.27$ &  $47.84 \pm 0.32$ &   $99.35 \pm 0.46$ \\
   66$<M_{12}<$166~\gev\    &  $14.36 \pm 0.17$ &  $25.87 \pm 0.24$ &  $38.22 \pm 0.28$ &   $78.44 \pm 0.41$ \\
   $M_{34}>$20~\gev\        &  $13.14 \pm 0.16$ &  $21.96 \pm 0.22$ &  $32.61 \pm 0.26$ &   $67.71 \pm 0.38$ \\
   66$<M_{34}<$116~\gev\    &  $10.80 \pm 0.15$ &  $17.45 \pm 0.19$ &  $27.12 \pm 0.24$ &   $55.38 \pm 0.34$ \\
...
~\fig{objSel-deltaRcutoff} shows the minimum \deltaR\ between any two leptons at
generator level in \fourlep\ events as a function of the mass of the \fourlep\
system and as a function of the transverse momentum of the leading \Z. A
\ZZllll\ \mc\ sample generated with \TGC s is used, with the anomalous coupling
\ffourg\ set to 0.1 and no form-factor applied, in order to give statistics at
high centre of mass energies. The plots show that the \deltaRlt{0.2} cut limits
sensitivity to events where the four-lepton system has invariant mass
$\mZZ>2$~\tev\ and to events where the leading \Z\ boson has high \pt,
$\pt(Z1)>900$~\gev. This is in the tails of the distributions expected with
\TGC\ couplings set near previous experimental limits, so this cutoff is not
expected to adversely affect the sensitivity of the \TGC\ limits.
...
\CZZ}, which is used to extrapolate from the number of observed events to the
true number of events in the fiducial \phasespace. The definition of the fiducial
\phasespace\ was given in~\sec{TheoryZZProduction-CxDef}. Formally, \CZZ\ is defined as:
\begin{equation}
...
which were generated inside the fiducial phase space (as given
in~\sec{TheoryZZProduction-CxDef}):
...
includes a contribution from events which are generated outside the fiducial volume but
pass all of the cuts at reconstruction level; for example an event with a generated lepton
with \pt\ just lower than the lepton \pt\ threshold which is reconstructed
with \pt\ just above the threshold. Since the numerator is not a subset of
...
it is necessary to assign the four generator-level leptons into pairs in order to apply the mass
cuts. There is no ambiguity in doing this in the \eemm\ channel as one simply
pairs the two electrons and the two muons, however as when dealing with events observed in data or
reconstructed MC, there is an ambiguity in assigning the pairs in the \eeee\ and \mmmm\ channels. One could use the generator documentation to determine which
leptons came from which \Z, but this is discouraged as it makes use of internal
details of the generator's calculation, which do not have a well defined physical meaning and may
vary from generator to generator. Instead,
the same algorithm is used as in the data and the reconstructed MC; that is,
picking the pairing that minimises the sum of the distances of the pairs from
the PDG \Z\ boson mass. It is found from applying this algorithm to the \eemm\
sample, ignoring lepton-flavour, that the number of events falling in the
fiducial volume is over-estimated by the algorithm with respect to the number
obtained by choosing the correct pair based on lepton-flavour by $2.84\pm0.03\%$
...
paragraph). These are obtained from the \powhegbox\ and \ggtwoZZ\ generators, as
described in~\sec{gen-comparisons}, with the relative contributions from the two generators
...
6\% lower in the \eeee\ final state at 7~\tev. This is attributed to the higher
...
20~\gev\ (see~\fig{el-id-eff-et}). The contamination from events outside the
fiducial volume is smaller for the \eemm\ final state as there is no
contribution from events generated outside the fiducial volume incorrectly passing the
\dilep\ mass cuts as a result of mis-pairing, since there is no mis-pairing in
in \eemm\ decays.
...
\small
    ...
    for the 8~\tev\ analysis.]{Details of the acceptance and selection
    efficiencies and contaminations, estimated from \mc,
    for the 8~\tev\ analysis. The first row shows the
    reconstruction factor \CZZ, defined as the ratio of the number of events
    passing the selection requirements (including contributions from $\ZZ
    \rightarrow \tau+X$ and events outside the fiducial region) to the number
    of generated events in the fiducial region.  The second row shows the
...
8~\tev\ analysis. In order to compare the acceptance between the two analyses,
...
but also to the relaxing of the \loosePP\ identification requirements in 2012, as
...
properties consistent with the Higgs boson. A \sm\ Higgs boson with such a mass will
...
acceptance \CZZ\ and the expected signal yield are discussed and estimated. %The uncertainties associated with reconstruction
...
in~\sec{systematic-estimate-tables}.

\subsubsection{Electron Systematic Uncertainties}

\begin{itemize}
     \item {\bf Reconstruction and Identification Efficiency:}
     ...
     due to the methods used to subtract the background, possible biases of the
    ...
    reproduced by the simulation, as discussed in~\sec{el-energy-calibration}),
...
\begin{itemize}
    ...
    obtained with and without the muon energy scale correction applied. For the
    8~\tev\ analysis, the systematic is estimated by varying the muon energy scale
    ...
    for the muon's inner detector and muon spectrometer track. Uncertainties are
...
Uncertainties arise on the measurement of the single-lepton trigger efficiency in data.
...
Theoretical uncertainties on \CZZ\ arise due to the choice of
...
to the MSTW08 PDF set. The reweighting is done by assigning an weight to each
...
%CT10 PDF set and scale \mZZ, 
%\pythia, generated using the MRST modified LO PDF
%set and scale set to the arithmetic mean of the squared transverse masses of the
%two outgoing particles ($(m^{2}_{T}(Z1)+m^{2}_{T}(Z2)/2$), 
A comparison is made between the default \powhegbox\ and \sherpa,
%generated using CTEQ6L1 PDF set and scale set to
%\nicefrac{1}{2}$(m^{2}_{}(Z1)+p^{2}_{T}(Z1)+m^{2}_{}(Z2)p^{2}_{T}(Z2))^{\frac{1}{2}$, and with a
...
\sherpa\ is taken as the systematic uncertainty; the number from \pythia\
...
\begin{table}[htbp]
    ...
    by the relative contribution to the fiducial \cx, as show in~\tab{theory-gg-frac}.
...
\subsection{Estimates of Systematic Uncertainties on \CZZ}
...
\begin{table}[htbp]
   ...
   \end{tabular}
   \caption[Systematic uncertainties on the reconstruction acceptance \CZZ\ for
   the 7~\tev\ analysis.]
   {Summary of systematic uncertainties on the reconstruction acceptance \CZZ\
...
the expected yields. The reconstruction uncertainties to the expected yield are identical to the
...
\begin{table}[htbp]
   ...
   \end{tabular}
   \caption[Systematic uncertainties on the expected signal yield.]
   {Summary of systematic uncertainties on the expected signal yield 
   ...
   see~\tabs{objSel-syst-seven}{objSel-syst-eight}.} 
