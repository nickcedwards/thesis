...
$N_{ZZ}$ (\LumiPassGRLTwentyEleven\ \ifb)	  & \llll\  & \eeee\ & \mmmm\ & \eemm\ \\
	 	 \hline
   ...
   Primary Z mass                &  69.21 $\pm$ 0.38  &  12.92 $\pm$ 0.16 &  21.6 $\pm$ 0.22  &  34.67 $\pm$ 0.28  \\ 
...
individual efficiencies for the four leptons to pass the lepton selection
requirements. $\alpha_{reco}$ is the reconstruction to 
...
\ZZllll\ sample passing the selection requirements by the number of those events
...
{\rm SF} }{ N^{{\rm MC\ Fiducial\ Volume}}_{{\rm Generated}\ ZZ} }
\end{equation}
...
$\ZZ\ra\tau\tau\ell\ell$ decays, and from events generated outwith the fiducial
volume (e.g. with a lepton with a \pt\ lower than the lepton \pt\ threshold) but
which are reconstructed as within; \CZZ\ is therefore not a true efficiency. 

\CZZ\ is calculated separately for each of the three final states (\eeee,
\mmmm\ and \eemm), and separately for the \ZZ\ and \ZZs\ selection and for the 7
\tev\ and 8 \tev\ analysis. Values for \CZZ\ are given in~\tab{objSel-czz}, as
well as the estimated contribution from tau decays and events generated outside
the fiducial volume. Also given is the true selection efficiency, that is \CZZ\
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Updated to 4.7fb-1 numbers
% Nick: Updated 26th July
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{table}[htbp]
\small
    \centering
	...
	\hline\hline
        ...
        % C_ZZ
        Reconstruction factor $C_{ZZ}$  & 
        \ZZSevenCzzZZEEEE $\pm$  \ZZSevenCzzStatZZEEEE &
        \ZZSevenCzzZZMMMM $\pm$  \ZZSevenCzzStatZZMMMM &
        \ZZSevenCzzZZEEMM $\pm$  \ZZSevenCzzStatZZEEMM &
        \ZZSevenCzzZZLLLL $\pm$  \ZZSevenCzzStatZZLLLL \\

          Tau contamination (\%)  & 
        \ZZSevenTauContaminationPercentageZZEEEE $\pm$  \ZZSevenTauContaminationPercentageStatZZEEEE &
        \ZZSevenTauContaminationPercentageZZMMMM $\pm$  \ZZSevenTauContaminationPercentageStatZZMMMM &
        \ZZSevenTauContaminationPercentageZZEEMM $\pm$  \ZZSevenTauContaminationPercentageStatZZEEMM &
        \ZZSevenTauContaminationPercentageZZLLLL $\pm$  \ZZSevenTauContaminationPercentageStatZZLLLL \\
         from $\ZZ \rightarrow \tau+X$   & & & & \\
        %%
         % Contaminations from outside fid
         Contamination (\%)   & 
        \ZZSevenOutsideFidContaminationPercentageZZEEEE $\pm$  \ZZSevenOutsideFidContaminationPercentageStatZZEEEE &
        \ZZSevenOutsideFidContaminationPercentageZZMMMM $\pm$  \ZZSevenOutsideFidContaminationPercentageStatZZMMMM &
        \ZZSevenOutsideFidContaminationPercentageZZEEMM $\pm$  \ZZSevenOutsideFidContaminationPercentageStatZZEEMM &
        \ZZSevenOutsideFidContaminationPercentageZZLLLL $\pm$  \ZZSevenOutsideFidContaminationPercentageStatZZLLLL \\
        from events outside fiducial region  & & & & \\

        % "True" C_ZZ
        True reconstruction factor      & 
        \ZZSevenTrueCzzZZEEEE $\pm$  \ZZSevenTrueCzzStatZZEEEE &
        \ZZSevenTrueCzzZZMMMM $\pm$  \ZZSevenTrueCzzStatZZMMMM &
        \ZZSevenTrueCzzZZEEMM $\pm$  \ZZSevenTrueCzzStatZZEEMM &
        \ZZSevenTrueCzzZZLLLL $\pm$  \ZZSevenTrueCzzStatZZLLLL \\

	\hline\hline
    \end{tabular}
    \caption{Details of the \ZZllll\ acceptance and selection
    efficiencies and contaminations, estimated from \mc. The first row shows the 
    the reconstruction
    factor \CZZ\, defined as the ratio of the number of events passing
    the selection requirements (including contributions from $\ZZ \rightarrow \tau+X$
    and events outside the fiducial region) to the number of generated
    events in the fiducial region.  
    The second row shows the percentage contamination from $\rightarrow \tau+X$ events, 
    and the third the 
    percentage
    contamination 
    from events falling outside of the fiducial region at generator
    level, but passing the selection due to the energy, momentum or track
    direction being mis-reconstructed. 
    The final row shows the ``true''
    reconstruction efficiency, with the contribution from $\ZZ
    \rightarrow \tau+X$ and events outside the fiducial region removed
    from the numerator.
    All errors shown are statistical only.}
    \label{table:objSel-czz}
\end{table}

\subsection{Mispairing rates}

\subsection{Effect of \deltaR\ requirement}
