...
\begin{enumerate}
    ...
    two possible ways of pairing the \fourlep\s into OS pairs. The pairing which
...
individual reconstruction and selection efficiencies for the \fourlep\s. 
...
ways of pairing the \fourlep\s to give two oppositely signed pairs. There are
...
leading \Z. A \ZZllll\ sample generated with aTGCs is used in order to
give statistics at high centre of mass energies, generated with $\ffourg=0.4$ and no
form-factor applied. The plots show that the \deltaRlt{0.2} cut limits sensitivity to events with $\mZZ>2$~\tev\ and to events
with $\pt(Z1)>900$~\gev.
