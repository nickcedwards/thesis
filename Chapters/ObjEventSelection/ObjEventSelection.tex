...
The event selection requirements are as follows:

\begin{enumerate}

    \item \bf{Trigger} The event must pass a hight \pt\ single electron or single
    muon trigger.
    ...
    \item \bf{Trigger match} At least one lepton with $\pT>20$~(25)~GeV for muons (electrons), must match to a trigger object, see
      Sec~\ref{sec:LeptonTrigger}, 8.1, 8.2.

    \item \emph{\bfseries Quadruplet Formation} There must be two same flavour,
    ...
    \item \emph{\bfseries ``Primary" $Z$ candidate} The Z candidate closest to the Z
    pole must satisfy the mass cut $66<m_{12}<116$~GeV.

    \item \emph{\bfseries ``Secondary" $Z$ candidate} The other $Z$ candidate is
