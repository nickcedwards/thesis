...
and~\ref{sec:reco-mu-triggers}. The trigger thresholds were progressively
...
object that caused the trigger to fire, as described above. Such leptons are referred to as 
...
required in the trigger. Trigger matched leptons (in both 2011 and 2012 data)
are required to have \ptgt{25}, cluster \modetalt{2.47} and pass the \mediumPP\
...
\Zll\ events. Events are triggered on using a `tag' lepton, then the
efficiency for the second `probe' lepton to fire the trigger is measured. Scale
factors are thus derived to correct the single lepton trigger efficiencies in
the \mc\ simulation to that observed in data. This is translated to an event
level trigger efficiency scale-factor using the following formula:
...
\end{equation}
 ...
 where $N_l$ is the number of leptons matching to a trigger object,
...
muon triggers is $0.2\%$~\cite{muonPG_EPS} and for electron triggers is $\sim1\%$~\cite{egammaPG_EPS}. 

The trigger efficiencies for \ZZ\ events passing the full event selection
described below, derived from simulation after applying the scale-factors
described above, are listed in
Table~\ref{tbl:triggerMCeff}.
