...
Electron-Positron Collider (LEP), abd is between 45 and 170 m below the ground.
...
contained within a single sturcture incorporating a twin bore superconducting
...
Protons are injected into the LHC from the SPS (Super Proton Synchrotron) at a
centre of mass energy of 450 \GeV. They are then accelerated to the desired
...
particles in their bunches. 

The rate at which collisions occur depends on the instantanious luminosity
$\mathcal{L}$ and the collision cross-section $\sigma$, ralted by:

\begin{equation}
\frac{dN}{dt} = \mathcal{L} \dot \sigma
\end{equation}
