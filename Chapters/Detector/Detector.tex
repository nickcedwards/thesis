...
ATLAS uses a three level triggering system to indentify when to read out the
...
results to theoretical models. ATLAS uses a custom detector simulator based on
\geant~\cite{Agostinelli2003250}. It takes as input the output of a Monte-Carlo generator as described
...
through the ATLAS detector, using GEANT4 to simulate their interaction with the
material of the detector. The response of the detector components is also
simulated, and simulated output signals in the same format as the digitised
detector output are produced. These are then reconstructed in the same was as
