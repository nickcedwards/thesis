...
different way. For all of the other generators compared in ??, \photos\ is used
to model QED radiaition, wheras \sherpa\ has it's own treatment. There are two
ways of applying QED radiation to a \ZZllll\ final state. The radiation can
either be applied from the full \llll\ multipole, distibuting the recoil within
that, or assuming two sepaeare dipoles $\Z[\ell\ell]$ and $\~[\ell'\ell']$ each
of which undergoes QED radiation independaently from each other. The latter
option leaves the mass of each dilepton pair invarient, wheras the latter only
leaves the mass of the four-lepton system invariant. \photos\ uses the
...
linesahpe in \sherpa\ samples generated using the two aproaches. It is not  clear which is the correct approach
