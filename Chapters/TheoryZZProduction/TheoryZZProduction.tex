...
\end{tabular}
\end{center}
\caption[\cx s calculated at NLO in QCD with MCFM version 6.3 for \ppZZ 
$\ra\eemm$.]{\CX z calculated at NLO in QCD with MCFM version 6.3 for \ppZZ 
$\ra\eemm$. The \cx\ for the $llll$ final state is equal to these \cx s multiplied 
by two. The columns labeled ``\partDF" give 
the error derived from the 52 CT10 error sets and the difference between CT10 
and MSTW2008, while the ones labeled `Scale' give the error from changing the 
...
Based on the studies above, \powhegbox\ is used as the nominal \mc\ generator to
model \qqZZllll.
\powhegbox\ generates events to NLO accuracy, and will give NLO accuracy in all
inclusive distributions not making requirements on the \pt\ of additional parton
emissions; most of the \ZZ\ distributions of interest fall into this category,
with the exception of distributions such as the \pt\ of the \ZZ\ system.
Additionally, the dependence on the choice of \fact\ and \renorm\ scales is
expected to be smaller for a NLO calculation. In addition, the \ggZZ\ generator
is used to model \ggZZllll. When estimating efficiencies and expected
distributions, the samples generated using \powhegbox\ and \ggZZ\ are combined,
normalised according to the \cx\ for the two processes; this is referred to as
\powhegbox+\ggZZ.

\sherpa\ is used as an alternative \mc\ generator to cross check efficiency estimates from
\powhegbox. \sherpa\ is chosen because although it is only a LO generator, it simulates real additional parton
emissions in the matrix-element, and so should give improved modelling of
kinematic distributions. \sherpa\ is also used in~\chap{TGCLimits} to estimate
the event yield in the presence of anomalous triple gauge couplings. 
...
One concrete example of a NP model giving rise to anomalous TGCs is
...
of the \cx\ is most apparent at high \ZZ\ invariant mass and transverse
...
pairs were selected, where the first dilepton pair must be composed of electrons or muons and
