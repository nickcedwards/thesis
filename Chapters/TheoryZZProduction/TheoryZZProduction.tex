...
\caption{Cross sections and acceptance calculated at NLO in QCD with MCFM version 6.1. The 
         ...
         shown are from Monte Carlo statistics. The \cx s shown are for the $ee\mu\mu$ final 
	 ...
	 ``CTEQ6.6 error set" gives the error derived from the 44 CTEQ6.6 error sets, while 
         ...
         $A_{ZZ}$, is the ratio of the \cx\  in $66<m_{12}<116$~GeV, $66<m_{34}<116$~GeV,
         $\pT(\ell)>7$~GeV, $\eta(\ell)|<2.7$ to the on-shell \cx.}
...
Each of the LEP experiments has set 95\% confidence limits on the \ZZZ\ and \ZZg\ anomalous
triple gauge couplings (aTGCs) $f_{i}^{V}$. The limits are set using the measured total
...
each decay channels. Each experiment first combines its limits across the different
...
combined negative log-likelihood, are shown in~\fig{lep-tgc}.
...
both the D0 and the CDF experiments. D0 measured the \ZZ\ \cx\ in
6.4~\ifb\ of data in the \ZZllll\ channel~\cite{} and 8.6~\ifb\ in the
\ZZllvv\ channel~\cite{Abazov:2012cj}. In the \ZZllll\ channel 10 events
were observed with an expected background of 0.37 $\pm$ 0.13. Combining the two
channels, the measured \cx\ for on-shell \ZZ\ production was
\measStatSyst{1.44}{\errAsym{0.31}{0.28}}{\errAsym{0.17}{0.19}}~pb, in 
agreement with the \sm\ prediction. The observed kinematic distributions were
also in good agreement with \sm\ predictions. CDF~\cite{CDF:2011ab}. CDF
...
the \sm\ prediction of \crossSec{p\bar p\ra ZZ}{1.4 \pm 0.1 \, pb}.

Both experiments have also set limits on aTGCs. D0 set limits using
...
Limits were set by comparing the expected yields with yields predicted in
samples simulated with different values of the anomolous couplings; this is used
to form a likelihood for that point. These are used to set 95\% confidence limits varying one
parameter at a time and holding the rest at zero. Two-dimenional limits are also
set.


%The \cx\ at the LHC for a centre-of-mass energy of 7 \TeV\ is predicted to be $\sim$5 times larger 
%than at the Tevatron. 
Recently the CMS experiment have measured the \ZZ\ production \cx~\cite{CMS-PAS-EWK-11-010} and found it consistent with the Standard Model prediction.
