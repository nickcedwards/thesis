...
\begin{itemize}
    ...
    associated uncertainties due to \partonDF\ and scale uncertainties.

    \item \powhegbox~\cite{Alioli:2010xd}, a general framework for implementing
    ...
    implementation of the \ZZ\ process in \powhegbox\ is given in~\cite{Melia:2011tj}.

    \item \ggtwoZZ~\cite{gg2ZZ}
    \item \sherpa~\cite{Gleisberg:2008ta}
    \item \pythia Version 6~\cite{pythia} at 7 tev, version 8~\cite{Sjostrand:2007gs} at 8 tev
    \item \herwig~\cite{Herwig}
    \item \herwigPP~\cite{Bahr:2008pv,}
    \item \mcatnlo~\cite{bib:mcatnlo} . Used to simulate the background processes \ttbar, \Wt
    and single-top at 7 \tev. Also WW, $WZ$ at 7 \tev.
    \item \alpgen~\cite{alpgen} is used to simulate the production of \W\ and \Z\ bosons in
    ...
    \item \ggtwoWW~\cite{Binoth:2006mf}
\end{itemize}
